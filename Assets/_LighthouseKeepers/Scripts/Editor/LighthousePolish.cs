 ...
 public static void Apply(){
  ...
  foreach(var name in LighthouseScenes.Names.Skip(1)){
   ...
   if(!GameObject.Find("LK Polish Applied")){
    ...
    foreach(var ps in Object.FindObjectsByType<ParticleSystem>())ps.GetComponent<ParticleSystemRenderer>().sharedMaterial=rain;
    if(name=="LK_Core"){
     ...
     foreach(var lamp in Object.FindObjectsByType<Light>()){var response=lamp.gameObject.AddComponent<DayEnvironmentResponse>();LighthouseScenes.Set(response,"practical",lamp);}
    }
    if(name.StartsWith("LK_Level")){
     ...
     foreach(var emitter in Object.FindObjectsByType<AudioSource>())if(emitter.name.Contains("Room-specific")){var response=emitter.gameObject.AddComponent<DayEnvironmentResponse>();LighthouseScenes.Set(response,level==2?"radio":"machinery",emitter);}
    }
   }
