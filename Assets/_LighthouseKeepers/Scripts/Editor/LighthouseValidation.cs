...
public static class LighthouseValidation {
 ...
 public static void Validate(){
  var errors=new List<string>();var notes=new List<string>{"Unity editor="+Application.unityVersion};
  ...
  var origins=UnityEngine.Object.FindObjectsByType<XROrigin>(FindObjectsInactive.Exclude,FindObjectsSortMode.None);if(origins.Length!=1)errors.Add("Expected one active XR Origin, got "+origins.Length);
  if(UnityEngine.Object.FindObjectsByType<AudioListener>(FindObjectsSortMode.None).Length!=1)errors.Add("Expected one listener");
  ...
  var sockets=UnityEngine.Object.FindObjectsByType<PuzzleSocket>(FindObjectsSortMode.None);foreach(var s in sockets)if(!s.Anchor||string.IsNullOrWhiteSpace(s.StationId)||string.IsNullOrWhiteSpace(s.Role))errors.Add("Incomplete socket: "+s.name);
  if(sockets.Select(s=>s.StationId).Distinct().Count()!=6)errors.Add("Expected six puzzle station identities");
  var thresholds=UnityEngine.Object.FindObjectsByType<FloodThreshold>(FindObjectsSortMode.None).Select(t=>t.Threshold).OrderBy(v=>v).ToArray();if(thresholds.Length!=4||thresholds.Distinct().Count()!=4)errors.Add("Flood thresholds missing or duplicate");
