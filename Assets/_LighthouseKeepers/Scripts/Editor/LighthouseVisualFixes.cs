...
public static class LighthouseVisualFixes {
 public static void Apply(){
  ...
  foreach(var name in LighthouseScenes.Names.Take(7)){
   ...
   foreach(var ps in Object.FindObjectsByType<ParticleSystem>()){var main=ps.main;main.startLifetime=.35f;main.startSize=.007f;var renderer=ps.GetComponent<ParticleSystemRenderer>();renderer.lengthScale=2;renderer.velocityScale=.015f;}
   ...
   if(name=="LK_Exterior"){
    ...
    foreach(var rock in Object.FindObjectsByType<MeshFilter>().Where(f=>f.name=="Jagged basalt outcrop")){
   ...
   foreach(var c in Object.FindObjectsByType<Collider>())if(!c.isTrigger&&!c.GetComponentInParent<Rigidbody>()&&!c.GetComponent<CharacterController>())c.gameObject.layer=8;
