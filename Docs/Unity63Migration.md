...
Final normal-folder verification: after preserving the 6000.5 Library, a clean 6000.3.23f1 import and `LighthouseValidation.Validate` completed with exit code 0 and `LIGHTHOUSE ASSET VALIDATION PASSED` (`/tmp/lk63-original-validation.log`). No tracked files changed during that import. Ten pre-existing untracked duplicate ProjectSettings files remain preserved and excluded from publication.
