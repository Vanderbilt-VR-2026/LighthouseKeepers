...
| Scenes | Single CastleRange entry with legacy fallback | LK_Bootstrap + six additive environment scenes and standalone LK_DevGym; no reference scene copies |
...
Safety checkpoint: `8bc7ccc` records preserved initial user changes and initial setup. Original changes are retained in InitialUserChanges.patch. OpenXR 1.18 validation queries the active platform in one Meta feature predicate; automatic rules are deferred until Android is selected.
